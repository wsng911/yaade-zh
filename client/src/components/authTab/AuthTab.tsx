...
export default function AuthTab({
  ...
  const generateToken = useCallback(async () => {
    if (!authData) {
      errorToast('认证数据缺失', toast);
      return;
    }

    if (authData.type !== 'oauth2' || !authData.oauth2) {
      errorToast('OAuth2数据缺失', toast);
      return;
    }
