...
const AccessTokenSettings: FunctionComponent = () => {
  ...
  async function handleAddAccessTokenClick() {
    try {
      ...
      if (res.status === 200) {
        ...
        successToast('访问令牌已创建', toast);
      } else if (res.status === 409) {
        errorToast('访问令牌已存在', toast);
      } else {
        throw new Error();
      }
    } catch (e) {
      errorToast('访问令牌创建失败', toast);
    }
  }
  ...
  async function handleDeleteAccessTokenClick(id: number) {
    try {
      ...
      successToast('访问令牌已删除', toast);
    } catch (e) {
      errorToast('访问令牌删除失败', toast);
    }
  }
