...
function Dashboard() {
  ...
  async function getRequestAccessTokenFromCode(
    ...
    try {
      ...
      successToast('令牌已生成', toast);
    } catch (e) {
      console.error(e);
      errorToast('获取令牌失败：' + e, toast);
    } finally {
    ...
    (request: RestRequest | WebsocketRequest) =>
      api
        ...
        .then((res) => {
          ...
          successToast('请求已保存', toast);
        })
        .catch((e) => {
          console.error(e);
          errorToast('请求保存失败', toast);
        }),
    [toast],
  );
  ...
  const updateCollection = useCallback(
    (collection: Collection | CurrentCollection) =>
      api
        ...
        .then((res) => {
          if (res.status !== 200) throw new Error();
          successToast('集合已保存', toast);
        })
        .catch((e) => {
          console.error(e);
          errorToast('集合保存失败', toast);
        }),
    [toast],
  );
  ...
  const updateScript = useCallback(
    (script: Script) =>
      api
        ...
        .then((res) => {
          if (res.status !== 200) throw new Error();
          successToast('脚本已保存', toast);
        })
        .catch((e) => {
          console.error(e);
          errorToast('脚本保存失败', toast);
        }),
    [toast],
  );
  ...
  const renameRequest = useCallback(
    async (id: number, name: string) => {
      try {
        ...
        successToast('请求已重命名', toast);
      } catch (e) {
        console.error(e);
        errorToast('请求重命名失败', toast);
      }
    },
  ...
  const deleteRequest = useCallback(
    async (id: number) => {
      try {
        ...
        successToast('请求已删除', toast);
      } catch (e) {
        console.error(e);
        errorToast('请求删除失败', toast);
      }
    },
  ...
  const duplicateRequest = useCallback(
    async (id: number, newName: string) => {
      try {
        ...
        successToast('请求已复制', toast);
      } catch (e) {
        console.error(e);
        errorToast('请求复制失败', toast);
      }
    },
    [collections, toast],
  );
  ...
  const duplicateCollection = useCallback(
    async (id: number, newName: string) => {
      try {
        ...
        successToast('集合已复制', toast);
      } catch (e) {
        console.error(e);
        errorToast('集合复制失败', toast);
      }
    },
    [toast],
  );
  ...
  const duplicateScript = useCallback(
    async (id: number, newName: string) => {
      try {
        ...
        successToast('脚本已复制', toast);
      } catch (e) {
        console.error(e);
        errorToast('脚本复制失败', toast);
      }
    },
    [collections, toast],
  );
  ...
  const deleteScript = useCallback(
    async (id: number) => {
      try {
        ...
        successToast('脚本已删除', toast);
      } catch (e) {
        console.error(e);
        errorToast('脚本删除失败', toast);
      }
    },
  ...
  const takeScriptOwnership = useCallback(
    async (id: number) => {
      try {
        ...
        successToast('您已成为此脚本的所有者', toast);
      } catch (e) {
        console.error(e);
        errorToast('获取脚本所有权失败', toast);
      }
    },
    [toast, user?.id],
  );
  ...
  const onRefreshResults = useCallback(async () => {
    ...
    try {
      ...
      successToast('已刷新', toast);
    } catch (e) {
      console.error(e);
      errorToast('刷新结果失败', toast);
    }
  }, [currentScript, toast]);

  useEventListener('message', handlePongMessage);

  let panel = <div>选择请求或集合</div>;
  ...
  return (
    <div className={styles.parent}>
      ...
      <Modal isOpen={isOpen} onClose={() => {}}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>扩展连接失败</ModalHeader>
          <ModalBody>
